...
const srcDir = path.resolve('data/combination');
const destDir = path.resolve('src/content/combinations');
...
function stripLeadingH1IfMatchesTitle(body, title) {
  ...
  // allow optional leading whitespace/newlines before the H1
...
async function processFile(file) {
  ...
  // Remove a leading H1 that duplicates the title
  ...
  if (parsed.data.ingredients) outputData.ingredients = parsed.data.ingredients;
